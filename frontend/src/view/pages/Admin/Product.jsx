...
const Product = () => {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/users/products/${productId}`);
    ...
    <Layout>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 w-full sm:w-auto">
          <div className="relative flex-grow mb-4 sm:mb-0 sm:mr-4 lg:w-[950px]">
            <input
              type="text"
              placeholder="Search"
      ...
      {products.map((product) => (
        <tr key={product._id} className="border-b">
          ...
          <td className="py-4 px-6 flex justify-start space-x-2">
            <button
              className="text-gray-600 hover:text-gray-900 ml-2" // Added margin-left to adjust positioning
